...
// Generate a proof for the `latest` tag of the `twitter` circuit.
// The `twitter` circuit name is specified in the `name` field of `./twitter/sindri.json`.
const circuitIdentifier = 'twitter:latest';
...
const proofInput = JSON.stringify({ username, pub_key, signature, hashed_message });
